...
char* indexes(const char* path)
{
    // create a string used to check if indexes exits 
    char* path_index = calloc(1, strlen(path) + strlen("index.php") + sizeof(char));
    if (path_index == NULL)
    {
        error(501);
    }
    ...
    // concatenate 
    strcat(path_index, "index.php");

    // checks to see if an index.php file exists inside directory passed in  
    ...
    // allocate new memory for html index 
    path_index = calloc(1, strlen(path) + strlen("index.html") + sizeof(char));
    if (path_index == NULL)
    {
        error(501);
    }
    ...
    // check to see if an index.html file exists in the directory 
    if (access(path_index, F_OK) == 0)
    {
        return path_index;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // make sure there is nothing in content 
    *content = NULL;
    
    // make sure length is pointing to zero
    *length = 0;
    ...
    // read the file byte by byte and store into buffer  
    while ( fread(&buffer, 1, sizeof(char), file) == 1 )
    {
        // increase counter 
        i++;
        
        // increase content to size +1 for null terminator 
...
const char* lookup(const char* path)
{
    ...
    // copy everything after last appearance of a period
    file_ext = strrchr(path, '.');
    
    // return the appropriate MIME
    // !strcasecmp() returns 1 if the strings are the same ignoring case
    if (!strcasecmp(".css", file_ext))
        return "text/css";
    
    else if (!strcasecmp(".html", file_ext))
        return "text/html";
    
    else if (!strcasecmp(".gif", file_ext))
        return "image/gif";
        
    else if (!strcasecmp(".ico", file_ext))
        return "image/x-icon";
        
    else if (!strcasecmp(".jpg", file_ext))
        return "image/jpeg";
    
    else if (!strcasecmp(".js", file_ext))
        return "text/javascript";
        
    else if (!strcasecmp(".php", file_ext))
        return "text/x-php";
        
    else if (!strcasecmp(".png", file_ext))
        return "image/png";
    else
        return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // initialize query 
    for (int i = 0, n = strlen(query) ; i < n ; i++)
    {
        query[i] = '\0';
    }
        
    // make sure only two spaces in request line 
    int spaces = 0;
    ...
    // ensure there is a space after GET  
    if (line[3] != ' ')
    {
        error(405);
        return false;
    ...
    // ensure request line begins with a '/'
    if (line[4] != '/')
    {
        error(501);
        return false;
    ...
    // ensure the version is HTTP/1.1
    if ( strcmp(strrchr(line,' '), " HTTP/1.1\r\n") != 0 )
    {
        error(505);
        return false;
    }
    ...
    // iterate through line until a "'?' or ' ' is reached
    for (i = 0; line[4+i] != '?' && line[4+i] != ' ' ; i++ ) 
    {
    ...
    // line[i] now is either a ? or a space 
    
    // if line[i] is a space there is no query
    if (line[4+i] == ' ')
    {
        query[0] = '\0';
        return true;
    ...
    // if line [i] is a ? there could be a query
    if (line[4+i] == '?')
    {
        // hop over the question mark
        i++;
        
        // if there is nothing after the ?, there is no query 
        if (line[4+i] == ' ')
        {
            query[0] = '\0';
            return true;
        ...
        {
            // keep track of query position
            int j;
            
            // iterate until last space is reached 
            for (j = 0; line[4+i+j] != ' ' ; j++) 
            {
                // make sure there are no quotations 
                if ( line[4+i+j] == '\"')   
                {
                    error(400);
                    return false;
                ...
                // copy characters from line into query 
                query[j] = line[4+i+j];
            }
